...
opts_chunk$set(echo = FALSE,
               message=FALSE,
               warning=FALSE, cache=FALSE, error=FALSE,
               fig.path = here("fig/"),
               dev = c("png"))
```
...
quantifier, interference, and typicality. Participant number and item number
...
prior_vals <- function(priors, class) {
  pr_pars <- priors$prior[priors$class == class]
  pr_pars <- strsplit(pr_pars, "(", fixed = TRUE)[[1]]
  name <- pr_pars[[1]]
  pars <- strsplit(pr_pars[[2]], ")")[[1]]
  pars <- lapply(strsplit(pars, ","), as.numeric)
  return(list(params = pars[[1]], name = name))
}

intercept <- prior_vals(priors, "Intercept")
slope <- prior_vals(priors, "b")
sd_ref <- prior_vals(priors, "sd")
cor_ref <- prior_vals(priors, "cor")
```
...
For the intercept we used a `r intercept$name` distribution with
$\mu = `r intercept$params[[1]]`$ and $\sigma =  `r intercept$params[[2]]`$. 
For the slopes we used a `r slope$name` distribution with
$\mu = `r slope$params[[1]]`$ and $\sigma =  `r slope$params[[2]]`$. 
...
$\mu = `r sd_ref$params[[1]]`$ and $\sigma =  `r sd_ref$params[[2]]`$. 

For the random effects correlation between the intercept and the slope we used
the LKJ distribution [@lewandowski2009gener; @stan2021] with $\eta = `r cor_ref$params`$.
...
intercept <- prior_vals(priors_binom, "Intercept")
slope <- prior_vals(priors_binom, "b")
sd_ref <- prior_vals(priors_binom, "sd")
cor_ref <- prior_vals(priors_binom, "cor")
...
For the intercept we also used a `r intercept$name` distribution, though a narrower one. The mean was 
$\mu = `r intercept$params[[1]]`$ and $\sigma =  `r intercept$params[[2]]`$. 
