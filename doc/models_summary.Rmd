...
rstan_models_smr <- dir_ls(here("models"),
                           regexp =  "t?gdur_stan_region[1-9].rds") %>%
  map(readRDS) %>%
  map(~ summary(.,
                pars = stanm_pars,
                probs = c(0.025, 0.975)
...
dir_ls(here("models"), regexp =  "gdur_stan_region[1-9].rds") %>%
...
dir_ls(here("models"),
       regexp =  "[^t]gdur_g?een_stan_region[1-9].rds",
       perl = TRUE) %>%
...
dir_ls(here("models"), regexp =  "tgdur_stan_region[1-9].rds") %>%
...
```{r tgdur-split }
dir_ls(here("models"),
       regexp =  "tgdur_g?een_stan_region[1-9].rds",
       perl = TRUE) %>%
