...
Model which assumes a mixture component on positive quantifiers only
...
model {
  ...
  for (i in 1:N){
    ...
    if (quant[i] == 1) target += lognormal_lpdf(rt[i] | mu, sigma_e2);
...
generated quantities{
  ...
  for (i in 1:N){
    ...
    if (quant[i] == 1) yrep[i] = lognormal_rng(pred_mu, sigma_e2);
  }
}
